...
As a post-v1 portability exercise, the same \texttt{core/} source has been cross-built \texttt{STANDALONE} for the WCH CH32V003 --- a roughly ten-cent RISC-V (RV32EC) microcontroller with 16\,KB of flash and 2\,KB of SRAM. A compile-time \emph{tiny profile} overrides the static maxima down to a single zone, sensor, and actuator and shrinks the reserved \texttt{thermal\_core\_t} from 4\,KB to 1\,KB, so the core state fits the 2\,KB SRAM. The whole \texttt{STANDALONE} firmware --- the portable core, a bit-banged DS18B20 plus TIM2-PWM and EXTI-tachometer BSP, and the \texttt{ch32fun} runtime --- links at \textbf{12{,}056\,bytes} of flash (74\,\% of 16\,KB) and \textbf{1{,}044\,bytes} of SRAM, a cross-build measurement gated in CI. An on-hardware bring-up of this port is follow-on bench work; what is established here is that the identical control core builds for, and fits, a part at the extreme low end of the cost range.
