...
| 0 | `no-heap-no-syscall` | links a guarded unit-test binary that wraps `malloc`/`free`/`open`/`read`/`write`/`mmap`/`sleep` etc. and fails if any are called from `core/`. Pairs with an `nm -u` symbol denylist over the portable `core/`, `protocol/`, and `support/` archives. Defends the PRD §4.2 portability promise. |
...
- **No-heap / no-syscall guard:** two complementary mechanisms, both gating from Stage 0.
  - **Static (authoritative):** `nm -u` over `core/`, `protocol/`, and `support/` portable archives, filtered through `ci/core-symbol-denylist.txt`, must produce no output. The denylist contains heap calls, syscalls, blocking sleeps, stdio, pthreads, wall-clock reads, and soft-float helper symbols. This is the canonical check — it answers "did a portable layer reach for any of these symbols?" without depending on call-graph attribution.
...
**Target detail:** v1 ships **ESP32-C3** (SuperMini board with USB-Serial-JTAG console).  C3 has working bench hardware in `/home/testpc/git-repos/claude/2026/tmp2/esp32-thermal-core`; other ESP32 RISC-V variants are future ports.  ESP32-S3 / ESP32-WROOM (Xtensa) are not v1 targets.
...
**Stage 18 status:** the tiny profile, the `platform/ch32v003/` STANDALONE port, and the `build-ch32` + `unit-tiny-profile` CI gates are shipped; Stage 18 is closed. The CH32V003 firmware is verified to cross-compile, link, and fit the part (flash 74 %, SRAM 51 %), and has since been **brought up on real hardware** — a heat/cool telemetry capture of the regulator running standalone on a CH32V003 (DS18B20 + Noctua NF-A8 fan) is committed at `docs/paper/data/ch32v003-capture.csv` and plotted in the white paper's Evaluation section. That bring-up is a functional demonstration (hand-applied heat, unregulated ambient); a calibrated benchmark sweep on instrumented hardware remains the documented follow-on. Stage 17 (the fan-health detector) is independent of Stage 18 and is now closed.
...
**Stage 19 status:** the CH32V003 host-command channel -- the `CH32_COMMAND` bench firmware, the `thermal-telemetry-tool` host driver, and the three-leg `build-ch32` + `telemetry-tool` CI gates -- is shipped; Stage 19 is closed. A post-close bench finding made the firmware RX interrupt-driven: a polled reader lost host-command bytes during the regulating tick's ~800 ms DS18B20 conversion blackout, so `bsp_ch32_uart.c` now feeds an RXNE-interrupt ring buffer (command build only). The shipping STANDALONE firmware is byte-for-byte unchanged. Stage 20 (fan-health on the CH32) follows once a PWM-to-RPM sweep has been captured on hardware with the tool.
...
- **Schema roundtrip:** the `fan_health` JSON block's json2static-emit ↔ config_jsmn-parse equality is covered by `test_fan_health_roundtrip` (via `configs/fan-health-demo.json`), so codegen and the host loader agree on the block layout. The actual CH32 configs are exercised end-to-end by the `build-ch32` CI matrix — both the active NF-A8 telemetry leg and the Arctic P8 alt-config leg run json2static with `--enable-fan-health --pinmap-prefix ch32` and link the result.
...
**Stage 20 status:** the Stage 17 fan-health detector is enabled in the CH32V003 STANDALONE firmware with a real, on-rig PWM-to-RPM baseline; the `build-ch32` matrix exercises the default (fan-health off), telemetry, and command variants on the active NF-A8 config plus a fourth `arctic` leg that links the Arctic P8 alt config with `--enable-fan-health` — two reference fan configs (NF-A8 active, Arctic P8 PWM PST as a drop-in alt) ship as worked examples. After the Fable-v2 review, the command/bypass bench image keeps fan-health compiled out by default because its job is PWM/RPM sweep control, while the telemetry and arctic legs still compile the detector in. After M6, the CH32 tiny profile also compiles PID out by default because the active CH32 configs are step-wise-only. Current measured sizes are: default 12 260 B flash / 1 044 B SRAM, telemetry+fan-health 14 420 B flash / 1 444 B SRAM, command 14 072 B flash / 1 508 B SRAM. Stage 20 is closed — and with it the first slice of the Stage 17 follow-on (a host-tool baseline-capture sweep + the MCU enablement) is shipped; the remainder (scenario directives, NVS persistence, the `thermalcore-fanhealth` CLI, scheduled-probe mode, BLOCKED detection, the 2D temperature-compensated baseline) stays deferred.

- **Post-review diagnostic update:** the CH32 telemetry tap reports ring overflow as `TSIG_PLATFORM_CH32_TELEMETRY_DROPS`, a cumulative canonical sample emitted on the next drain after records are dropped. The telemetry+fan-health leg is back under the PRD soft target after M6's PID compile gate, and the command leg has materially more headroom because fan-health and PID are no longer part of that bench-control image unless explicitly requested with `CH32_COMMAND_FAN_HEALTH=1` / `CH32_ENABLE_PID=1`.
